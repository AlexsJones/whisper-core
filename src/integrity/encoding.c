...
jnx_uint8 *encode_from_string(jnx_char *input, jnx_size input_length, 
...
jnx_char *decode_to_string(jnx_char *input, jnx_size input_length,
    jnx_size *output_length) {
  jnx_encoder *e = jnx_encoder_create();
  jnx_char *decoded = jnx_encoder_b64_decode(e,input,input_length,output_length);
  jnx_encoder_destroy(&e);
  return decoded;
}
