...
int listen_for_socket_fd(peer *remote_peer, session *ses) {
  ...
  ses->is_connected = 1;
  ses->secure_receiver_listen_socket = sock;
  return fd;
}
...
void secure_comms_end(session *s) {
  if (s->secure_socket) {
    ...
    if (s->secure_receiver_listen_socket > 0) {
      JNXLOG(LDEBUG, "Shutting down secure comms receiver listener fd %d",
             s->secure_receiver_listen_socket);
      close(s->secure_receiver_listen_socket);
    }
    s->secure_socket = -1;
  }
}
